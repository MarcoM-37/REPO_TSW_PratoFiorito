...
<template>
    <div :key="item.id" class="slot_oggetto">
        ...
        <div class="azioni-item" v-if="!listaAcquisti.includes(item.id_oggetto)">
            ...
            <button class="btn-prova" @click="$emit('prova',item)"> {{ inProva.sfondo === item.id_oggetto ? '↩️ Ripristina' : '👁️ Prova' }} </button>
            <button class="btn-compra" @click="$emit('acquisto',item)"> 💰 {{ item.prezzo }} </button>
        </div>

        <!-- Se è già acquistato -->
        <div class="azioni-item" v-else>
            <button class="btn-prova" @click="$emit('prova',item)">✨ Equipaggia</button>
